<?php
    ...
    if(isset($_POST['update'])){
        ...
        if(move_uploaded_file($_FILES['passport']['tmp_name'], $passport_folder) && move_uploaded_file($_FILES['receipt']['tmp_name'], $receipt_folder)){
            ...
            $update_profile = $connectdb->prepare("UPDATE users SET first_name = :first_name, last_name = :last_name, whatsapp = :whatsapp, other_number = :other_number, res_state = :res_state, user_email = :user_email, passport = :passport, payment_receipt= :payment_receipt, payment_status = 1, reg_number = :reg_number, gender =:gender WHERE user_id = :user_id");
